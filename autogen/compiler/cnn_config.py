...
OUT_DIR = ROOT / "autogen" / "compiler" / "out"
OUT_COMPILER_SOURCE_C = ROOT / "autogen" / "compiler" / "cnn_accel.c"
OUT_COMPILER_DATA_ASM = OUT_DIR / "dcache_init.S"
OUT_COMPILER_ASM = OUT_DIR / "cnn_accel.S"
OUT_COMPILER_ICACHE_HEX = OUT_DIR / "icache_initial.hex"
OUT_COMPILER_DCACHE_HEX = OUT_DIR / "dcache_initial.hex"
OUT_COMPILER_TESTCASES_DIR = OUT_DIR / "testcases"
...
def compiler_testcase_dir(sample_index: int) -> Path:
    return OUT_COMPILER_TESTCASES_DIR / f"sample{sample_index}"


def compiler_testcase_paths(sample_index: int) -> tuple[Path, Path, Path, Path, Path]:
    case_dir = compiler_testcase_dir(sample_index)
    return (
        case_dir / f"dcache_init_sample{sample_index}.S",
        case_dir / "cnn_accel.S",
