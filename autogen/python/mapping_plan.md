...
current accelerator design in
`../../cv32e40p/fpga/rtl/src/accelerator_sim.sv`.
...
## Programmability Workflows

The mapping is available through two software workflows:

- Manual low-level API workflow: Python emits RV32 assembly directly through
  `accelerator_api.py` and `cnn_top_emit.py`. This path is used when exact
  scheduling, explicit overlap, and instruction-level control matter most.
- C/compiler workflow: user code can include `../compiler/accel_runtime.h` and
  call accelerator operations from freestanding C. The checked-in
  `../compiler/cnn_accel.c` program uses the same layer schedule, links with a
  generated D-cache initialization assembly file, and is compiled by the RISC-V
  GCC toolchain. This path is more programmable and still gets speed from
  accelerator CONV/FC calls, but the compiler may keep some helpers out of line
  to fit the 8 KiB I-cache image. Handwritten C can be built with
  `python3 autogen/compiler/build_c_program.py`.

Both workflows have separate verifiers:

- `python3 autogen/python/verify_cnn.py` checks the manual assembly workflow.
- `python3 autogen/compiler/verify_cnn.py` checks the C/compiler workflow.

Each verifier checks golden-model agreement, no compressed instructions, SRAM
image bounds, and stale generated files for its own workflow.
